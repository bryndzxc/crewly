...
export default function Index() {
    ...
    const onPerPageChange = (nextPerPage) => {
        ...
        router.get(
            route('developer.access_requests.index'),
    ...
    const columns = useMemo(
        () => [
            ...
            { key: 'full_name', label: 'Name', className: 'px-4 py-3' },
            { key: 'company_name', label: 'Company', className: 'px-4 py-3' },
            { key: 'email', label: 'Email', className: 'px-4 py-3' },
            { key: 'phone', label: 'Mobile', className: 'px-4 py-3' },
            { key: 'employee_count_range', label: 'Employees', className: 'px-4 py-3 whitespace-nowrap' },
            { key: 'requested_plan', label: 'Plan', className: 'px-4 py-3 whitespace-nowrap' },
            { key: 'industry', label: 'Industry', className: 'px-4 py-3' },
            { key: 'status', label: 'Status', className: 'px-4 py-3 whitespace-nowrap' },
            { key: 'details', label: 'Details', className: 'px-4 py-3' },
    ...
    const approve = (leadId) => {
        ...
        router.post(
            route('developer.access_requests.approve', leadId),
            {},
            {
                preserveScroll: true,
                onFinish: () => {
    ...
    const decline = (leadId) => {
        ...
        router.post(
            route('developer.access_requests.decline', leadId),
            {},
            {
                preserveScroll: true,
                onFinish: () => {
    ...
    const detailsText = (lead) => {
        const lines = [];
        const current = String(lead.current_process || '').trim();
        const pain = String(lead.biggest_pain || '').trim();
        const message = String(lead.message || '').trim();

        if (current) lines.push(`Current process: ${current}`);
        if (pain) lines.push(`Biggest pain: ${pain}`);
        if (!current && !pain && message) lines.push(message);
    ...
    return (
        <AuthenticatedLayout user={auth.user} header="Developer Settings" contentClassName="max-w-none">
            <Head title="Developer Settings / Access requests" />
            ...
            <div className="space-y-4">
                ...
                <div className="mt-2">
                    <Table
                        ...
                        items={leads}
                        rowKey={(l) => l.id}
                        emptyState={<div className="text-sm">No access requests found.</div>}
                        pagination={
                            leadsPaginator
                                ? {
                        ...
                        renderRow={(lead) => (
                            <tr className="align-top">
                                ...
                                <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="text-sm text-slate-700">{lead.employee_count_range || '—'}</div>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                    <div className="text-sm text-slate-700">{(lead.requested_plan || '—').toString().toUpperCase()}</div>
                                </td>
                                <td className="px-4 py-3">
                                    <div className="text-sm text-slate-700">{lead.industry || '—'}</div>
                                </td>
                                <td className="px-4 py-3 whitespace-nowrap">
                                ...
                                <td className="px-4 py-3">
                                    <div className="text-sm text-slate-700 whitespace-pre-wrap break-words max-w-2xl">{detailsText(lead) || '—'}</div>
                                </td>
